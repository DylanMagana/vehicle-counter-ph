...
from detection import CentroidTracker, run_specialist
...
def _create_output_dirs(base: Path, classes: list[str]) -> dict[str, Path]:
    if base.exists():
        shutil.rmtree(base)
    dirs: dict[str, Path] = {
        "image":  base / "Output_image",
        "bytype": base / "Output_image_bytype",
        "report": base / "Output_report",
        "rpt_bt": base / "Output_report_bytype",
    }
...
def main() -> None:
    ...
    dirs = _create_output_dirs(OUTPUT_DIR, CLASSES)
    ...
    spec_trackers: list[CentroidTracker] = []
    for spec_cfg in SPECIALISTS:
        spec_models.append(YOLO(spec_cfg.model_path))
        spec_trackers.append(CentroidTracker())
    ...
    summary_writer = cv2.VideoWriter(
        str(OUTPUT_DIR / "summary.mp4"), fourcc, fps, (width, summary_h)
    )
    ...
    while True:
        ...
        if frame_idx % (FRAME_SKIP + 1) == 0:
            ...
            for spec_cfg, spec_model, tracker in zip(SPECIALISTS, spec_models, spec_trackers):
                w_clss, supp_boxes, supp_cnfs, supp_ids = run_specialist(
                    spec_cfg, spec_model, tracker, frame, CLASSES, w_xyxy, w_clss,
                )
                ...
                all_supp_labels[cls_idx] = supp_boxes
            ...
            for track_id, box, cls_idx, conf in zip(all_ids, all_xyxy, all_clss, all_cnfs):
                ...
                if inside and track_id not in crossed_ids:
                    ...
                    records.append(VehicleRecord(
                        filename=str(dirs["bytype"] / label / fname),
        ...
        print(f"\r[{bar}] {pct:5.1%} | {frame_idx}/{total} frames processed", end="", flush=True)
    ...
    write_report1(dirs["report"] / "report1.csv", bucket_counts, total_secs, BUCKET_SECONDS)
    write_rpt_vehbytype(dirs["rpt_bt"] / "rpt_vehbytype.csv", records)
